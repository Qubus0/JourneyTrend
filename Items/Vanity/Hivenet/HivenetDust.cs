...
namespace JourneyTrend.Items.Vanity.Hivenet
{
    public class HivenetDust : ModDust
    {
        ...
        {
            var oldAlpha = dust.alpha;
            dust.alpha = (int) (dust.alpha * 1.05);
            if (dust.alpha == oldAlpha) dust.alpha++;
            if (dust.alpha >= 255)
            {
                dust.alpha = 255;
                dust.active = false;
            ...
            if (dust.alpha > 100) dust.frame = new Rectangle(0, Main.rand.Next(4) * 10, 8, 10);
